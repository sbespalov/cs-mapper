...
public class MappingConfig {
	...
	private Map<String, BeanMapping> mappings = new HashMap<String, BeanMapping>();
	...
	public BeanMapping getBeanMapping(
										...
										String sourceClass,
										String mappingId) {
		String key = createMappingKey(sourceClass, targetClass, mappingId);
		BeanMapping result = mappings.get(key);
		if (result == null) {
			key = createMappingKey(sourceClass, targetClass, null);
	...
	public BeanMapping getBeanMapping(
		...
		if (result == null
			...
			synchronized (mappings) {
				SimpleMappingBuilder mappingBuilder = targetClass.isAssignableFrom(sourceClass) ? new SimpleMappingBuilder(mappingProfile, targetClass, sourceClass) : new SimpleMappingBuilder(mappingProfile, sourceClass, targetClass);
